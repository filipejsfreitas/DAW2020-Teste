...
const baseUrl = 'http://clav-api.di.uminho.pt/v2/classes';

(async function() {
    const processosBase = await axios.get(baseUrl + '/c900?token=' + token);

    const processosNivel3 = [];

    for(let filho of processosBase.data.filhos) {
        const processosNivel2 = await axios.get(baseUrl + '/c' + filho.codigo + '?token=' + token);

        for(let filho of processosNivel2.data.filhos) {
            processosNivel3.push(filho.codigo);
        }
    }

    return [ processosNivel3, processosNivel3.length ];
})()
